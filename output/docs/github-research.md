...
```text
MENU -> BIDDING -> LANDLORD_REVEAL -> PLAYING -> FINISHED
```

地主揭晓是独立的 3 秒阶段，期间显示皇冠和中央倒计时，不允许提前出牌。状态包含三方手牌、角色、余额、个人出牌区、底牌、当前玩家、上一手有效牌/玩家、连续 pass 数、出牌历史、叫分、成功出牌手数、炸弹数和胜负。任何 UI 交互只调用 `GameState` 动作；DOM 和 AI 都不直接修改手牌数组。异步 AI 与倒计时还会校验 `roundId`、`revision` 与会话令牌，防止重开后的旧计时器继续落子。
...
- 经典绿色牌桌，左右两名电脑、底部麒麟、顶部底牌与阶段信息；三位玩家各有靠近头像的独立出牌区，牌保留到本人下一回合开始。
- CSS transition/keyframes 完成发牌、选牌抬起、出牌滑入、动态炸弹投掷、火箭发射、飞机起飞和胜负结算；尊重 `prefers-reduced-motion`。
- 音效只使用 Web Audio API 程序合成，设置中支持音乐、音效和 0～30 倍音量增益；不得复用调研仓库中的音频。
- 布局兼容常见桌面、移动横屏与窄竖屏；手牌支持点击、长按滑动多选和键盘方向键；可选记牌器会根据真人手牌与成功出牌历史实时扣减各点数。
